...
### G8. Scenario 3: Morning run, with 3a calendar intake and 3b transcript intake (45 minutes) [build sheets done, Gerson to do the clicking]
Branch `gerson/g8-morning-run`. All three sheets are written. Build 3a first: it plus the check in row in scenario three is everything task G7's scenario four needs before it can be tested at all. Input `{ company_id, day }`. When run on the schedule, `day` is the previous working day.
