...
## Current position
- Last update: Saturday 19 September 2026, 20:00 Madrid. Marcus is away from about 20:00 for three to five hours.
...
- Checkpoint one, 20:00: passed with one item carried forward. The whole story runs on fake data. On live data every page loads from Gerson's database. The real interview turn is carried forward because Gerson's interview scenario is not finished.
- Biggest risk: Gerson's coding assistant ran out of allowance at about 19:25. Marcus's CLI is building scenarios 7 and 8 in Gerson's make.com account (zone eu1) through the MCP server `make-gerson`, agreed with Gerson. Gerson builds the interview turn and day summary scenarios by hand.
- Scenario 7, suggest workflows: built and switched on in make.com. Pull request 22, reviewed, pass. Blocked on Gerson pasting `supabase/migrations/0004_team_history.sql` into the Supabase SQL editor. After that the CLI runs one real test call (the line to paste is in the development manager's chat and repeated below), then pull request 22 can merge.
- Scenario 8, decision and draft creation: the draft template exists in make.com ("TEMPLATE: Weekly client status report", switched off) and migration 0005 is written on branch `marcus/g12-draft`. Blocked on two things from Gerson: pasting migration 0005, and creating a Make connection named "Scout Make" from an API token, because the account has no way to create a scenario without one. Then the CLI continues from step 7 of `cli_prompts/g12_decision_and_draft.md`.
- While Marcus is away the CLI runs `cli_prompts/while_marcus_is_away.md`: push the scenario 8 branch as a draft pull request, and build submit and day approval as direct database writes when scenario 6 is absent. Its report is appended to `cli_prompts/overnight_report.md`.
- Line to paste into the CLI once Gerson confirms migration 0004 ran: "The SQL in supabase/migrations/0004_team_history.sql has been applied to the live database by Gerson. Switch to branch marcus/g11-suggest and continue from the TEST section of ../cli_prompts/g11_suggest_scenario.md: run the one correct test call, confirm the result, finish the write up and update the pull request."
- In Marcus's note, not yet used: the SLNG API key for task M9. Missing from `frontend/.env.local`: the interview webhook address and the six other webhook addresses not yet built. Present: database address and key, demo sign in, the suggest webhook address and the shared secret.
- When Marcus is back, in order: read the CLI's report, check what Gerson finished, first live interview turn if his scenario answers, test scenarios 7 and 8, then decide with the development manager what is cut. Feature freeze was planned for 22:30 and will have passed: only finishing and testing what is already started, nothing new. Task M9 (SLNG voice) is cut unless everything else is green on Sunday before 09:45. Task M10 (pitch) still needs Marcus to read the draft aloud and choose the closing line.
