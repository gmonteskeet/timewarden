...
import { listCandidatesSince, reviewHistory } from "@/lib/data";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { candidates, complete } = await listCandidatesSince(since);
    return Response.json({ ok: true, candidates_created: reply.candidates_created, candidates, complete, since });
  } catch (error) {
